function largestProductinaSeries(n: number) {
  ...
  for (let i: number = 0; i <= 1000 - n; i++) {
    let currentProduct: number = arrProduct(slice(thousandDigits, i, n));
...
function arrProduct(arr: number[]): number {
...
function slice(
  ...
  return arr.slice(currentIndex, currentIndex + sliceSize);
}
